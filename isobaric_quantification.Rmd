...
```{r plot-msmsScore, fig.cap="Density plot of msmsScore with dashed line indicating the baseline, un-optimized cutoff.", echo=FALSE, fig.asp=0.6}
# Plot default filters
...
ggplot() +
  geom_density(aes(x = msmsScore, color = isDecoy), 
               fill = NA, na.rm = TRUE) +
  geom_vline(xintercept = 2, lty = "longdash") +
  annotate("segment", x = 2.2, xend = 2.7, y = 0.75, yend = 0.75,
           colour = "darkgrey", size = 2, arrow = arrow(type = "open")) +
...
```{r plot-mass-error, fig.cap="Density plot of absParentMassErrorPPM with dashed line indicating the baseline, un-optimized cutoff.", echo=FALSE, fig.asp=0.6}
ggplot() +
  geom_density(aes(x = absParentMassErrorPPM, color = isDecoy), 
               fill = NA, na.rm = TRUE) +
  geom_vline(xintercept = 10, lty = "longdash") +
  annotate("segment", x = 9, xend = 5.5, y = 0.17, yend = 0.17,
           colour = "darkgrey", size = 2, arrow = arrow(type = "open")) +
  scale_y_continuous("Density", expand = expansion(mult = c(0, 0.05))) +
  scale_x_continuous(expand = expansion(0)) +
  theme_bw(base_size = 12)
...
```{r plot-num-pep, fig.cap="Density plot of peptides_per_1000aa. Proteins with values greater than 1 are selected by default prior to optimization. The plot area has been zoomed in.", echo=FALSE, fig.asp=0.6}
